...
export default function FinanceLoginPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Unable to send login link');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4 pt-20">
      ...
      >
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-cyan-300">Private workspace</p>
        <h1 className="mt-3 text-2xl font-bold text-white">Finance login</h1>
        <p className="mt-2 text-sm leading-6 text-slate-500">
          We will email a one-time link to {FINANCE_LOGIN_EMAIL}. Opening it signs you in and keeps you signed in on
          this browser.
        </p>
        <button type="submit" className={`${financeStyles.primary} mt-6 w-full`} disabled={submitting}>
