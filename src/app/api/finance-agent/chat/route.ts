        ...
        try {
          ...
          for (let round = 0; round < MAX_TOOL_ROUNDS; round += 1) {
            ...
            const needsAddForm =
              ...
              !pendingActions.some((action) => action.actionType === 'ledger_entry_add');
            ...
            for (const call of response.message.tool_calls) {
              ...
              try {
                ...
                console.error('Could not persist finance agent tool log:', logError);
              }
              messages.push({
                role: 'tool',
          ...
          if (!finalText) {
            finalText = 'I reached the safe reasoning limit. Please try a more focused request.';
          }
          ...
          try {
            ...
            console.error('Could not persist finance agent error message:', persistError);
          }
          send({
            type: 'error',
            error: message,
...
function sanitizeHistory(value: unknown) {
  ...
  for (const item of candidate) {
    ...
    const id = 'id' in item && typeof item.id === 'string' && item.id ? item.id : randomUUID();
    const createdAt =
...
function parseToolArguments(raw: unknown, toolName?: string): Record<string, unknown> {
  ...
  try {
    const relaxed = text.replace(/'/g, '"').replace(/,\s*([}\]])/g, '$1');
...
function writeIntentFor(content: string): 'ledger_entry_add' | 'ledger_entry_update' | null {
  const normalized = content.toLowerCase();
  const mentionsEntry = /\b(ledger|ledgers|entr(?:y|ies)|transaction|transactions|expense|income|transfer)\b/.test(
    normalized
  );
  if (mentionsEntry && /\b(add|create|new|insert|log|open(?:\s+the)?\s+form)\b/.test(normalized)) {
    return 'ledger_entry_add';
  }
  if (mentionsEntry && /\b(edit|update|change|modify)\b/.test(normalized)) {
    return 'ledger_entry_update';
  }
  return null;
}

function requiredToolFor(content: string, writeIntent: ReturnType<typeof writeIntentFor>) {
  if (writeIntent === 'ledger_entry_add') {
    return monthFromMessage(content) ? 'ledger_entry_add' : 'ledgers_list';
  }
  const normalized = content.toLowerCase();
  const requestedMonth = monthFromMessage(content);
  if (requestedMonth && /\bledger|ledgers|transaction|transactions|account(?:s)?\b/.test(normalized)) {
    return 'ledger_get';
  }
  if (/\bledger|ledgers|transaction|transactions|account(?:s)?\b/.test(normalized)) {
    return 'ledgers_list';
  }
  if (/\bsnapshot|snapshots\b/.test(normalized)) {
    return 'snapshots_list';
  }
  if (/\bportfolio|holding|holdings|balance|balances|bank|banks|fund|funds\b/.test(normalized)) {
...
function draftMonthFromToolMessages(messages: GroqMessage[]) {
    ...
    try {
      ...
      const rows = parsed.filter(
        (row): row is { month: string; status?: string } =>
          typeof row === 'object' && row !== null && 'month' in row && typeof row.month === 'string'
      );
...
async function openLedgerAddForm(input: {
  ...
  const month =
    monthFromMessage(input.userContent) || draftMonthFromToolMessages(input.messages) || (await latestDraftMonth());
  if (!month) {
    return { pendingAction: undefined, error: 'No draft ledger found' };
  }
  const startedAt = Date.now();
  const arguments_ = { month, ...ledgerAddPrefill(input.userContent) };
  try {
    const result = await executeFinanceTool('ledger_entry_add', arguments_);
    try {
      await logAgentToolCall({
    ...
    const error = cause instanceof Error ? cause.message : 'Could not open add form';
    try {
      await logAgentToolCall({
        requestId: input.requestId,
        model: input.model,
...
function ledgerAddPrefill(content: string) {
  const prefill: Record<string, unknown> = {};
  if (/\bincome\b|\bpayment\s+from\b/i.test(content)) {
    prefill.type = 'income';
  }
  if (/\b(?:tbo|the byte office)\b/i.test(content)) {
    prefill.accountName = 'TBO';
  }
  const payer = content.match(/\b(?:payment\s+)?from\s+([\p{L}][\p{L}\p{M}' -]*?)(?:\s+for\s+|[.!?]|$)/iu)?.[1]?.trim();
  const purpose = content.match(/\bfor\s+([\p{L}][\p{L}\p{M}' -]*?)(?:[.!?]|$)/iu)?.[1]?.trim();
  if (payer || purpose) {
    prefill.note = [payer ? `Payment from ${payer}` : 'Payment', purpose ? `for ${purpose}` : '']
      .filter(Boolean)
      .join(' ');
  }
  if (purpose) prefill.category = titleCase(purpose);

  const amount = content.match(/(?:\b(?:pkr|rs\.?)\s*|₨\s*)([\d,]+(?:\.\d+)?)/i)?.[1];
  if (amount) {
    const value = Number(amount.replace(/,/g, ''));
    if (Number.isFinite(value) && value > 0) prefill.amount = value;
  }
  return prefill;
}
...
function buildResponse(content: string, actions: PendingAgentAction[], model: string): FinanceAgentResponse {
  return {
    message: {
      id: randomUUID(),
      role: 'assistant',
