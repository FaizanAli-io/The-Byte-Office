...
const ledgerEntryProperties = {
  ...
  type: {
    type: 'string',
    enum: ['income', 'expense', 'transfer', 'fund_contribution', 'fund_withdrawal'],
  },
  accountId: {
    type: 'string',
    description: 'Stable account UUID when known',
  },
  accountName: {
    type: 'string',
    description: 'Account name or shorthand stated by the user, such as TBO or The Byte Office',
  },
...
export const financeAgentTools: GroqTool[] = [
  tool('portfolio_get', 'Get the live portfolio, stable item IDs, balances, and PKR total.', {}),
  tool('snapshots_list', 'List up to 50 saved portfolio snapshots, newest first.', {}),
  tool('snapshot_get', 'Get one saved portfolio snapshot by its stable ID.', { id: { type: 'string' } }, ['id']),
  tool('ledgers_list', 'List monthly ledgers with their status and last update time.', {}),
  tool(
    'ledger_get',
  ...
  tool(
    ...
    'Immediately open an edit form prefilled with an existing draft ledger entry. Use entrySerial from ledger_get (for example, 0001); entryId is only for internal compatibility. Do not ask the user to retype fields. The user submits the form to save. Do not claim the entry was updated.',
    {
      ...
      entrySerial: {
        type: 'string',
        pattern: '^\\d{4,}$',
        description: 'The entry serial returned by ledger_get, such as 0001',
      },
      entryId: { type: 'string', description: 'Internal entry UUID' },
      ...ledgerEntryProperties,
    },
    ['month']
  ),
